...
glob('!(node_modules|public)/**/*.{mjs,js,jsx}').then(files =>
  files.forEach(async file => {
    const content = (await fs.readFile(file)).toString()
    if (!content) return

    const [_, js, css] = content.match(/^(.+?)(?:<style>(.+?)<\/style>)?\s*$/s)
    let result = prettier.format(js, {
      semi: false,
      singleQuote: true,
      parser: 'babel'
    })
    if (css)
      result += `\n<style>\n${await comb.processString(css.trim())}</style>\n`
    if (result !== content) fs.writeFile(file, result)
  })
)
