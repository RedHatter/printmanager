...
import {
  ...
  Button, Input, MenuItem, Checkbox, ListItemText, Typography, Paper
...
function CreateModal ({ clients, salesmen, files, onClose, deleteFiles, updateJob, ...props }) {
  let initalModel = {
    client: '',
    salesman: '',
    dropDate: [ null, null ],
  ...
  function setModel (values) {
    if ((values.client || values.listType) && client && listType) {
      model.trackingNumber = clients.find(c => c._id == client)
        .trackingNumbers[listType.toLowerCase()]
    }
  ...
  return (
    <Dialog open className="create-modal" PaperComponent={ Paper }
      PaperProps={ {
        component: Form,
        onSubmit: () => Promise.all([deleteFiles(selectedFiles), updateJob(model)]).then(onClose),
    ...
    >
        <DialogContent className="content">
          <Grid container spacing={ 16 }>
            ...
            { editMode && <Fragment>
              <Grid item sm={ 6 }>
                <TextField required fullWidth label="Name" pattern={ /^[A-Z]{3,5} \d{4}-\d+/ }
                  value={ name } onChange={ e => setModel({ name: e.target.value }) } />
              </Grid>
              <Grid item sm={ 6 }>
                <TextField fullWidth select label="Art Status" value={ artStatus } onChange={ e => setModel({ artStatus: e.target.value }) }>
            ...
            <Grid item sm={ 6 }>
              <TextField required value={ client } onChange={ e => setModel({ client: e.target.value }) } label="Client" select fullWidth>
                { clients.map(c => <MenuItem value={ c._id } key={ c._id }>{ c.name }</MenuItem>) }
              </TextField>
            </Grid>
            <Grid item sm={ 6 }>
              <TextField required value={ salesman } onChange={ e => setModel({ salesman: e.target.value }) } label="Salesman" select fullWidth>
            ...
            <Grid item sm={ 6 }>
              <TextField required fullWidth label="Type" select value={ jobType } onChange={ e => setModel({ jobType: e.target.value }) }>
                { enums.jobType.map(value => <MenuItem key={ value } value={ value }>{ value }</MenuItem>) }
              </TextField>
            </Grid>
            <Grid item sm={ 6 }>
              <TextField required fullWidth label="List type" select value={ listType } onChange={ e => setModel({ listType: e.target.value }) }>
            ...
            <Grid item sm={ 4 }>
              <NumberFormat fullWidth pattern={ /^\+1 \(\d{3}\) \d{3} \d{4}$/ } label="Tracking number" type="tel"
                ...
                value={ trackingNumber } onValueChange={ format => setModel({ trackingNumber: format.value }) } />
            </Grid>
            <Grid item sm={ 4 }>
              <TextField fullWidth required label="Vendor" value={ vendor } onChange={ e => setModel({ vendor: e.target.value }) } />
            </Grid>
            <Grid item sm={ 4 }>
              <NumberFormat fullWidth required isNumericString thousandSeparator label="Quantity" customInput={ TextField }
                value={ quantity } onValueChange={ format => setModel({ quantity: format.value }) }/>
            </Grid>
            <Grid item sm={ 6 }>
              <TextField required fullWidth label="Size" select value={ size } onChange={ e => setModel({ size: e.target.value }) }>
                { enums.size.map(value => <MenuItem key={ value } value={ value }>{ value }</MenuItem>) }
              </TextField>
            </Grid>
            <Grid item sm={ 6 }>
              <TextField required fullWidth label="Envelope" select value={ envelope } onChange={ e => setModel({ envelope: e.target.value }) }>
                { enums.envelope.map(value => <MenuItem key={ value } value={ value }>{ value }</MenuItem>) }
              </TextField>
            </Grid>
            <Grid item sm={ 6 }>
              <TextField value={ addons } onChange={ e => setModel({ addons: e.target.value }) }
            ...
            <Grid item sm={ 6 }>
              <TextField required fullWidth label="Postage" select value={ postage } onChange={ e => setModel({ postage: e.target.value }) }>
                { enums.postage.map(value => <MenuItem key={ value } value={ value }>{ value }</MenuItem>) }
              </TextField>
            </Grid>
            <Grid item sm={ 12 }>
              <TextField fullWidth multiline rows={ 3 } label="Comments" value={ comments } onChange={ e => setModel({ comments: e.target.value }) } />
            </Grid>
            { editMode && <Fragment>
              { _id in files &&
                <Grid item sm={ 12 }>
              ...
              <Grid item sm={12}>
                <FormControlLabel label="Force completed" control={
                  <Checkbox checked={ forceComplete }
                    onChange={ e => setModel({ forceComplete: e.target.checked }) } />
                } />
              </Grid>
            </Fragment> }
          </Grid>
